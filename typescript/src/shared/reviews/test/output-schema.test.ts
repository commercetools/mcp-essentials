...
describe('Reviews Output Schema', () => {
  describe('Paged Response Schema', () => {
    it('should validate real API paged response', () => {
      const result = reviewsPagedSchema.safeParse(outputData);
  ...
  describe('Individual Entity Schema', () => {
    it('should validate individual entities from results', () => {
      ...
      if (results && results.length > 0) {
        ...
        const result = reviewsSchema.passthrough().safeParse(firstEntity);
    ...
    it('should validate all entities in results array', () => {
      ...
      results.forEach((entity: any, index: number) => {
        const result = reviewsSchema.passthrough().safeParse(entity);
  ...
  describe('Read Output Schema (Union)', () => {
    it('should validate paged response', () => {
      const result = readReviewOutputSchema.safeParse(outputData);
  ...
  describe('Schema Structure', () => {
    it('should be a valid Zod schema', () => {
      ...
      expect(readReviewOutputSchema).toBeDefined();
      expect(reviewsPagedSchema).toBeDefined();
    });
  });
});
