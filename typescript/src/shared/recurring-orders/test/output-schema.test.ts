import {
  recurringOrdersSchema,
  recurringOrdersPagedSchema,
  readRecurringOrdersOutputSchema,
} from '../output-schema';
import outputData from './output.data.json';

describe('RecurringOrders Output Schema', () => {
  describe('Paged Response Schema', () => {
    it('should validate real API paged response', () => {
      const result = recurringOrdersPagedSchema.safeParse(outputData);
  ...
  describe('Individual Entity Schema', () => {
    it('should validate individual entities from results', () => {
      ...
      if (results && results.length > 0) {
        ...
        const result = recurringOrdersSchema.passthrough().safeParse(firstEntity);
    ...
    it('should validate all entities in results array', () => {
      ...
      results.forEach((entity: any, index: number) => {
        const result = recurringOrdersSchema.passthrough().safeParse(entity);
  ...
  describe('Read Output Schema (Union)', () => {
    it('should validate paged response', () => {
      const result = readRecurringOrdersOutputSchema.safeParse(outputData);
  ...
  describe('Schema Structure', () => {
    it('should be a valid Zod schema', () => {
      expect(recurringOrdersSchema).toBeDefined();
      expect(readRecurringOrdersOutputSchema).toBeDefined();
      expect(recurringOrdersPagedSchema).toBeDefined();
    });
  });
});
