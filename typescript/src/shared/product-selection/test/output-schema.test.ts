...
describe('ProductSelection Output Schema', () => {
  ...
  describe('Individual Entity Schema', () => {
    it('should validate individual entities from results', () => {
      ...
      if (results && results.length > 0) {
        ...
        const result = productSelectionSchema.passthrough().safeParse(firstEntity);
    ...
    it('should validate all entities in results array', () => {
      ...
      results.forEach((entity: any, index: number) => {
        const result = productSelectionSchema.passthrough().safeParse(entity);
  ...
  describe('Read Output Schema (Union)', () => {
    it('should validate paged response', () => {
      const result = readProductSelectionOutputSchema.safeParse(outputData);
  ...
  describe('Schema Structure', () => {
    it('should be a valid Zod schema', () => {
      expect(productSelectionSchema).toBeDefined();
      expect(readProductSelectionOutputSchema).toBeDefined();
      expect(productSelectionPagedSchema).toBeDefined();
    });
  });
});
