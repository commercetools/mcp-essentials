import {
  standalonePriceSchema,
  standalonePricePagedSchema,
  readStandalonePriceOutputSchema,
} from '../output-schema';
import outputData from './output.data.json';

describe('StandalonePrice Output Schema', () => {
  describe('Paged Response Schema', () => {
    it('should validate real API paged response', () => {
      const result = standalonePricePagedSchema.safeParse(outputData);
  ...
  describe('Individual Entity Schema', () => {
    it('should validate individual entities from results', () => {
      ...
      if (results && results.length > 0) {
        ...
        const result = standalonePriceSchema.passthrough().safeParse(firstEntity);
    ...
    it('should validate all entities in results array', () => {
      ...
      results.forEach((entity: any, index: number) => {
        const result = standalonePriceSchema.passthrough().safeParse(entity);
  ...
  describe('Read Output Schema (Union)', () => {
    it('should validate paged response', () => {
      const result = readStandalonePriceOutputSchema.safeParse(outputData);
  ...
  describe('Schema Structure', () => {
    it('should be a valid Zod schema', () => {
      expect(standalonePriceSchema).toBeDefined();
      expect(readStandalonePriceOutputSchema).toBeDefined();
      expect(standalonePricePagedSchema).toBeDefined();
    });
  });
});
