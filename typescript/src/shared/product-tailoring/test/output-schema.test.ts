import {
  productTailoringSchema,
  productTailoringPagedSchema,
  readProductTailoringOutputSchema,
} from '../output-schema';
import outputData from './output.data.json';

describe('ProductTailoring Output Schema', () => {
  describe('Paged Response Schema', () => {
    it('should validate real API paged response', () => {
      const result = productTailoringPagedSchema.safeParse(outputData);
  ...
  describe('Individual Entity Schema', () => {
    it('should validate individual entities from results', () => {
      ...
      if (results && results.length > 0) {
        ...
        const result = productTailoringSchema.passthrough().safeParse(firstEntity);
    ...
    it('should validate all entities in results array', () => {
      ...
      results.forEach((entity: any, index: number) => {
        const result = productTailoringSchema.passthrough().safeParse(entity);
  ...
  describe('Read Output Schema (Union)', () => {
    it('should validate paged response', () => {
      const result = readProductTailoringOutputSchema.safeParse(outputData);
  ...
  describe('Schema Structure', () => {
    it('should be a valid Zod schema', () => {
      expect(productTailoringSchema).toBeDefined();
      expect(readProductTailoringOutputSchema).toBeDefined();
      expect(productTailoringPagedSchema).toBeDefined();
    });
  });
});
